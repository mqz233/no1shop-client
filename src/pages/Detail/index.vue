          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <!-- 切换选中状态：排他思想 先全部改成同一状态 然后再单独改选中那个 -->
                <dd
                  changepirce="0"
                  ...
                  :key="value.id"
                  @click="
                    changeSelectedIndex(index, spuSaleAttr.spuSaleAttrValueList)
                  "
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 用户输入双向绑定 -->
                <!-- 同时要监测用户输入是否合法 用change事件比blur事件好 change会检测输入是否有变化 -->
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="skuNum"
...
export default {
  ...
  methods: {
    changeSelectedIndex(index, list) {
    ...
    // 添加购物车步骤
    // 1.给服务器发请求，通知服务器要购买的商品和数量
    // 2.服务器返回添加情况，成功下一步，失败展示失败原因（使用promise）
    // 3.跳转到添加购物车成功页面（需要传递商品数量 和 商品详情：使用sessionStorage）
    async addShopCart() {
      try {
