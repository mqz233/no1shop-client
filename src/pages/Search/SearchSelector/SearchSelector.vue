      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(value, index) in item.attrValueList" :key="index">
            <!--  -->
            <a
              @click.prevent="searchByProp(item.attrId, value, item.attrName)"
              >{{ value }}</a
            >
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    searchByTrademark(trademark) {
      ...
      this.$emit("search-by-trademark", trademark);
    },
    searchByProp(id, value, name) {
      // 自定义事件 让父组件Search去处理请求
      this.$emit("search-by-prop", id, value, name);
    },
  },
  computed: {
